import { parseTdf } from '@tom/core';
export class TournamentService {
    dbService;
    constructor(dbService) {
        this.dbService = dbService;
    }
    async ingestTdf(tournamentId, rawXml) {
        ...
        // Upsert Tournament
        await this.dbService.run(`INSERT INTO tournaments (id, name, city, state, country, organizer_name, organizer_pop_id, round_time, start_date, current_round, raw_tdf, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET
        ...
        for (const p of Object.values(data.players)) {
            ...
            await this.dbService.run(`INSERT INTO players (id, tournament_id, user_id, first_name, last_name, full_name, birth_date)
         VALUES (?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
        ...
        for (const pod of data.pods) {
            for (const r of pod.rounds) {
                for (const m of r.matches) {
                    ...
                    const existing = await this.dbService.queryOne('SELECT status, p1_reported_winner, p2_reported_winner, confirmed_winner_id, is_tie FROM matches WHERE id = ?', [matchId]);
                    ...
                    const confirmedWinner = existing?.confirmed_winner_id ||
                        (m.outcome === '1' ? m.player1Id : m.outcome === '2' ? m.player2Id : null);
                    const isTie = existing?.is_tie ?? (m.outcome === '3' ? 1 : 0);
                    await this.dbService.run(`INSERT INTO matches (id, tournament_id, pod_category, round_number, table_number, player1_id, player2_id, tom_outcome, status, p1_reported_winner, p2_reported_winner, confirmed_winner_id, is_tie, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(id) DO UPDATE SET
        ...
        for (const sp of data.standings) {
            for (const rank of sp.rankings) {
                ...
                await this.dbService.run(`INSERT INTO standings (id, tournament_id, pod_category, player_id, place)
           VALUES (?, ?, ?, ?, ?)`, [standingId, tournamentId, sp.category, rank.playerId, rank.place]);
            }
        }
        return data;
    }
    async getPlayerActiveMatch(tournamentId, popId) {
        const tourney = await this.dbService.queryOne('SELECT current_round FROM tournaments WHERE id = ?', [tournamentId]);
        if (!tourney)
            return null;
        const roundNumber = tourney.current_round;
        const match = await this.dbService.queryOne(`SELECT * FROM matches
       WHERE tournament_id = ? AND round_number = ? AND (player1_id = ? OR player2_id = ?)`, [tournamentId, roundNumber, popId, popId]);
        if (!match)
            return null;
        const isPlayer1 = match.player1_id === popId;
        const opponentId = isPlayer1 ? match.player2_id : match.player1_id;
        const playerRow = await this.dbService.queryOne('SELECT * FROM players WHERE tournament_id = ? AND user_id = ?', [tournamentId, popId]);
        const opponentRow = opponentId
            ? await this.dbService.queryOne('SELECT * FROM players WHERE tournament_id = ? AND user_id = ?', [tournamentId, opponentId])
            : null;
        return {
            matchId: match.id,
            tournamentId,
    ...
    async getStandings(tournamentId, category) {
        let sql = `
    ...
    `;
        ...
        const rows = await this.dbService.queryAll(sql, params);
        return rows.map((r) => ({
            place: r.place,
    ...
    async getRoundPairings(tournamentId, roundNumber) {
        ...
        const rows = await this.dbService.queryAll(`SELECT * FROM matches
       WHERE tournament_id = ? AND round_number = ?
       ORDER BY table_number ASC`, [tournamentId, finalRound]);
        const pairings = [];
        for (const m of rows) {
            const p1 = await this.dbService.queryOne('SELECT * FROM players WHERE tournament_id = ? AND user_id = ?', [tournamentId, m.player1_id]);
            const p2 = await this.dbService.queryOne('SELECT * FROM players WHERE tournament_id = ? AND user_id = ?', [tournamentId, m.player2_id]);
            pairings.push({
                matchId: m.id,
