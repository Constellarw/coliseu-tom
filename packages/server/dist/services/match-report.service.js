export class MatchReportService {
    ...
    async reportResult(params) {
        ...
        const match = await this.dbService.queryOne('SELECT * FROM matches WHERE id = ?', [matchId]);
        if (!match) {
        ...
        await this.dbService.run(`UPDATE matches
       SET status = ?,
           p1_reported_winner = ?,
           p2_reported_winner = ?,
           confirmed_winner_id = ?,
    ...
    async getReportQueue(tournamentId) {
        return this.dbService.queryAll(`SELECT m.*,
       ...
       ORDER BY m.round_number DESC, m.table_number ASC`, [tournamentId]);
    }
}
//# sourceMappingURL=match-report.service.js.map