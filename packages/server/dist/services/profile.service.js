export class ProfileService {
    ...
    async getPlayerStats(popId) {
        const cleanId = popId.trim();
        // 1. Total distinct tournaments
        const tourneyRow = await this.dbService.queryOne('SELECT COUNT(DISTINCT tournament_id) as count FROM players WHERE user_id = ?', [cleanId]);
        // 2. Completed matches breakdown
        const matches = await this.dbService.queryAll(`SELECT confirmed_winner_id, is_tie, status
       FROM matches
       WHERE (player1_id = ? OR player2_id = ?) AND status = 'CONFIRMED'`, [cleanId, cleanId]);
        let wins = 0;
        let ties = 0;
        let losses = 0;
        ...
        const standings = await this.dbService.queryAll('SELECT place FROM standings WHERE player_id = ? ORDER BY place ASC', [cleanId]);
    ...
    async getTournamentHistory(popId) {
        const cleanId = popId.trim();
        // Get all tournaments where player was registered
        const tourneys = await this.dbService.queryAll(`SELECT DISTINCT t.id, t.name, t.start_date, t.city
       FROM tournaments t
       JOIN players p ON p.tournament_id = t.id
       WHERE p.user_id = ?
       ORDER BY t.start_date DESC, t.updated_at DESC`, [cleanId]);
        const result = [];
        for (const t of tourneys) {
            // Total players in tournament
            const countRow = await this.dbService.queryOne('SELECT COUNT(*) as count FROM players WHERE tournament_id = ?', [t.id]);
            // Placement
            const standingRow = await this.dbService.queryOne('SELECT place FROM standings WHERE tournament_id = ? AND player_id = ? LIMIT 1', [t.id, cleanId]);
            // Matches in this tournament
            const matchRows = await this.dbService.queryAll(`SELECT 
        ...
        ORDER BY m.round_number ASC`, [t.id, cleanId, cleanId]);
            let wins = 0;
            let ties = 0;
            let losses = 0;
            const matches = [];
