...
export function parseTdf(xmlContent: string): TournamentData {
  ...
  // Parse players
  const players: Record<string, TomPlayer> = {};
  ...
  for (const pod of rawPods) {
    ...
    for (const r of rawRounds) {
      ...
      for (const m of rawMatches) {
        ...
        const outcome = String(m['@_outcome'] ?? '0').trim() as MatchOutcomeCode;
  ...
  // Parse standings
  const standings: TomPodStanding[] = [];
  ...
  for (const stPod of rawStandingsPods) {
    ...
    const rankings = rawStandingsPlayers.map((sp: any) => ({
